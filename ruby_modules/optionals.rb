...
def provision_optionals (vm:, registry_config:, repository_config:)
  if registry_config["enabled"] == true
    provision_registry(vm: vm, host: registry_config["host"], ip: registry_config["external_ip"])
  end
  if repository_config["enabled"] == true
    provision_repository(vm: vm, host: repository_config["host"], ip: repository_config["external_ip"])
  end
end

def provision_repository(vm:, host:, ip:)
...
def provision_registry(vm:, host:, ip:)
